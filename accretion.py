...
#%%
# @numba.njit
def accretion_disc(N, x_start, x_end, dt, v, ite):
    ...
    fx = np.array([[-0.001*x[i]/(r[i,j]**3+0.001) for i in range(N)] for j in range(N)])
...
trial = accretion_disc(71, -20, 20, 0.001, 0.2, 50)
...
#%%
plt.close('all')
plt.quiver(xx,yy,trial[1][2],trial[2][2])